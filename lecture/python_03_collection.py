...
print(list(set(a)))   # 순서 없어서 무작위로 나옴
...
#  - 멤버함수: update(), get(), keys(), valuse(), items()
...
print(dict_a)
import pprint
pprint.pprint(dict_a)  # dict_a 적은대로 이쁘게 출력 pycharm에선 x
...
b = {"b": 3,
     "c": 5}
a.update(b)  # 병합시 중복 key는 입력값(b)이 우선
print(a)    # {'a': 1, 'b': 3, 'c': 5} 출력
...
# in() : ()안의 key값이 존재 확인
print("c" in abc)   # True
print("f" in abc)   # False

# get() : 값 접근
# list, tuple, dict 접근
#  -> 컬레션[index or key]
#  -> ex: a[1], b["c"]
# print(abc["f"])   #"b"의 key값인 2출력  # key가 없으면 Error
