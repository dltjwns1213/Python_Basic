...
#   => 15줄 코드를 함수로 정의 하고 함수로 실행(효율적)

# 함수 개발 가이드 라인
# 1. 함수 이름
#       + 힘수 내용은 가능하면 짧게 작성(line 줄이기)
#       + 함수 이름에 함수의 역할과 의도를 명확히 드러낼 것!
# ex)  def print_hello():
#        print("hello")
# 2. 함수의 역할
#        + 하나의 함수에는 유사한 역할를 하는 코드만 포함
#        + 함수는 한 가지 기능만 명확히 정의
# ex) def add_value(x,y):
#       return x+y
# 3. 함수를 만들어야 하는 경우
#       + 공통적으로 사용되는 코드는 함수로 생성
#       + 복잡한 로직이 사용되는 경우 식별 가능한 이름의 함수로 작성
...
# 7. parameter와 return은 사용하지 않을 수 도 있음
#    (입력과 반환이 없는 함수도 존재)
...
# Defaulf Parmeter
#   - 만약에 함수호출시 입력해야하는 parmeter가 empty인 경우
#   - Defaulf Parmeter로 대체해주는 기능!
#   - 끝에서부터 채워야함
...
#   - teturn만 사용하면함수 호출문으로 None 값 전달
...
# Variable length parmeter(가변 길이 인자)
# - 인자(parameter): 함수 입력값
