...
def review_collector(movie_code, last_date):
    ...
    for item in review_list:
        ...
        # 1. 오늘 리뷰 건수 - DB 저장 된 리뷰건수(삭제 된 리뷰를 고려X)
        #   수집건수만큼 수집하고 멈추기
        ...
        if review_date == "조금전":
            ...
            review_date = datetime.now() - timedelta(minutes=reg_minute)  # 현재시간 -?분
            review_date = review_date.strftime("%Y. %m. %d. %H:%M")
        elif review_date[-3:] == "시간전":
            reg_hour = int(re.sub(r"[^~0-9]", "", review_date))
            review_date = datetime.now() - timedelta(hours=reg_hour)  # 현재시간 -?시간
            review_date = review_date.strftime("%Y. %m. %d. %H:%M")
        ...
        cllect_date = int(re.sub(r"[^~0-9]", "", review_date))
        if int(last_date) >= cllect_date:
            continue
        ...
        # MariaDB에 저장
        #  1) DB에 보낼 데이터 만들기
        # Tip: key값 -> Table coulmn(열)과 동일하게
        data = {
            "title": movie_title,
